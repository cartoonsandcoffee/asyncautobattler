...
const OUTPUT_PATH := "res://item_export.tsv"
...
var _E       # Enums class     — for reading enum values (E.TriggerType.ON_HIT etc.)
...
func _run() -> void:
	...
	print("Scanning: " + ITEMS_DIR)
	var files := _collect_tres_files(ITEMS_DIR)
	print("Found %d .tres files" % files.size())
	if files.is_empty():
		push_error("No .tres files found — check ITEMS_DIR path")
		return
	...
	for path in files:
		...
		if item == null:
			push_warning("Skipped (not an Item): " + path)
			continue
		rows.append(_item_to_row(item, path))
	...
	file.close()
	print("! Exported %d items → %s" % [rows.size() - 1, OUTPUT_PATH])


# ── Header ────────────────────────────────────────────────────────────────────
func _make_header() -> PackedStringArray:
	return PackedStringArray([
		"File Name", "Name", "Bundle", "Item Description",
		"HP Bonus", "Shield Bonus", "Damage Bonus", "Agility Bonus", "Strikes Bonus", "Burn Bonus",
		"Cant Strike", "Rarity", "Item Type", "Categories", "Keywords", "Rules",
	])


# ── Row ───────────────────────────────────────────────────────────────────────
func _item_to_row(item: Item, path: String) -> PackedStringArray:
	return PackedStringArray([
		path.get_file().get_basename(),
		item.item_name,
		_bundle_str(item.item_bundle),
		_strip_bbcode(item.item_desc),           # raw field — no Enums dependency
...
# ── Enum → string (all inline, no Enums autoload) ────────────────────────────
