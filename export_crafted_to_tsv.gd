...
# ── CONFIGURE ────────────────────────────────────────────────────────────────
const RECIPES_DIR := "res://Resources/CraftingRecipes/"
const OUTPUT_PATH := "res://crafted_items_export.tsv"
const ENUMS_PATH  := "res://Scripts/Autoloads/Enums.gd"   # update if path differs
const CRAFTED_RARITY := 8                        # Enums.Rarity.CRAFTED
# ─────────────────────────────────────────────────────────────────────────────
...
func _run() -> void:
	...
	for path in files:
		var recipe = ResourceLoader.load(path) as CraftingRecipe
		if recipe == null:
			push_warning("Skipped (not a CraftingRecipe): " + path)
			skipped += 1
			continue
		if recipe.result_item == null:
			push_warning("Skipped (no result_item): " + path)
			skipped += 1
			continue
		if recipe.result_item.rarity != CRAFTED_RARITY:
			continue  # not a CRAFTED item — skip silently
	...
	file.close()
	print("✅ Exported %d crafted items → %s  (%d skipped)" % [rows.size() - 1, OUTPUT_PATH, skipped])
...
# ── Row ───────────────────────────────────────────────────────────────────────
func _recipe_to_row(recipe: CraftingRecipe, path: String) -> PackedStringArray:
	var item: Item = recipe.result_item

	# Component item names — ingredient_2 may be null (single-ingredient recipe)
	var component_names: Array[String] = []
	if recipe.ingredient_1:
		component_names.append(recipe.ingredient_1.item_name)
	if recipe.ingredient_2:
		component_names.append(recipe.ingredient_2.item_name)

	# Component bundles — unique values in order
	var seen_bundles: Dictionary = {}
	var bundle_list: Array[String] = []
	for ingredient in [recipe.ingredient_1, recipe.ingredient_2]:
		if ingredient == null:
			continue
		var b := _bundle_str(ingredient.item_bundle)
	...
	return PackedStringArray([
		...
		str(item.cant_attack),
		_item_type_str(item.item_type),
...
func _item_type_str(t: int) -> String:
	match t:
		0:  return "Weapon"
		1:  return "Body Armor"
		2:  return "Shield"
