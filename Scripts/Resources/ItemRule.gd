...
enum ConditionValueType {
	VALUE,				# A specified value / any number
	STAT_VALUE,			# The value of a stat from either entity
	STATUS_VALUE,		# the stacks of a status from either entity
	TRIGGER_AMOUNT		# The amount of the previous value that triggered this rule (ie, the first time you gain shield, double it)
}
...
## NONE means it will trigger on ANY stat gain/loss
...
## The threshold for Gain/Loss of stats and statuses. Ie: 3 = "When you gain 3 or more" or 1 = "Whenever you gain 1 or more"
@export var trigger_value_threshold: int = 0
...
## 1.0 = gain resource exact amount of source, 0.5 = gain half, 2.0 = gain twice
...
## How much target per source (1:2 = 2.0)
...
func _validate_property(property: Dictionary) -> void:
	...
	if prop_name == "trigger_status":
		if trigger_type not in [Enums.TriggerType.ON_STATUS_GAINED, Enums.TriggerType.ON_STATUS_REMOVED, Enums.TriggerType.ON_ENEMY_STATUS_GAIN, Enums.TriggerType.ON_ENEMY_STATUS_PROC]:
			property.usage = PROPERTY_USAGE_NO_EDITOR
	
	# Only show trigger_status for status-based triggers
	if prop_name == "trigger_value_threshold":
		if trigger_type not in [Enums.TriggerType.ON_STATUS_GAINED, Enums.TriggerType.ON_STATUS_REMOVED, Enums.TriggerType.ON_STAT_GAIN, Enums.TriggerType.ON_STAT_LOSS]:
			property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if prop_name in ["condition_stat_type", "condition_party_stat"]:
		if has_condition and compare_to != ConditionValueType.STAT_VALUE:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	if prop_name in ["condition_party_status"]:        
		if has_condition and compare_to != ConditionValueType.STATUS_VALUE:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	if prop_name in ["condition_to_party"]:        
		if has_condition and (compare_to == ConditionValueType.VALUE):
			property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if prop_name in ["target_item_category"]:
		if effect_type != Enums.EffectType.ADD_REPEATS:
			property.usage = PROPERTY_USAGE_NO_EDITOR
	...
	if prop_name == "effect_stat_party":
		if effect_of == ConditionValueType.VALUE || effect_of == ConditionValueType.TRIGGER_AMOUNT:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	# Show effect_stat_party, effect_stat_type, effect_stat_value only when effect_of is STAT_VALUE
	if prop_name in ["effect_stat_type", "effect_stat_value"]:
	...
	# Hide Effect fields for CONVERT
	if prop_name in ["effect_stat_party", "effect_stat_type", "effect_stat_value", "effect_status_value", "target_type", "effect_amount", "effect_of"]:
...
func get_desc_condition() -> String:
	...
	var comparison_string: String = ""
	var compare_entity_value: String = ""
	...
	condition_string = "If "
	comparison_string = get_comparison_string(condition_comparison)

	if condition_of == Enums.TargetType.SELF && condition_type == StatOrStatus.STAT:
		compare_entity_value = "your"
	elif condition_of == Enums.TargetType.SELF && condition_type == StatOrStatus.STATUS:
		compare_entity_value = "you have"
	elif condition_of == Enums.TargetType.ENEMY && condition_type == StatOrStatus.STAT:
		compare_entity_value = "your enemy's"
	elif condition_of == Enums.TargetType.ENEMY && condition_type == StatOrStatus.STATUS:
		compare_entity_value = "your enemy has"

	if condition_type == StatOrStatus.STAT:
		condition_string += compare_entity_value + " " + Enums.get_stat_string(condition_stat) + " is " + comparison_string
	if condition_type == StatOrStatus.STATUS:
		condition_string += compare_entity_value + " " + comparison_string + " " + Enums.get_status_string(condition_status) 

	if compare_to == ConditionValueType.VALUE:
		value_string = str(condition_value) + " "
	if compare_to == ConditionValueType.STAT_VALUE:
		value_string = Enums.get_target_string(condition_to_party) + " " + Enums.get_stat_type_string(condition_stat_type) + " " + Enums.get_stat_string(condition_party_stat)
	elif compare_to == ConditionValueType.STATUS_VALUE:
		value_string = Enums.get_target_string(condition_to_party) + " " + Enums.get_status_string(condition_party_status)

	condition_string = condition_string.replace("[BLANK]", value_string)
