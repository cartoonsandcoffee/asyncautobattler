...
@onready var set_grid: GridContainer = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelSets/ScrollContainer/gridSets
@onready var scroll_container: ScrollContainer = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelItems/ScrollContainer

@onready var set_panel: PanelContainer = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelSets
@onready var item_panel: PanelContainer = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelItems
...
@onready var bundle_shame: Button = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelFilters/VBoxContainer/VBoxContainer/HBoxContainer/boxBundles/bunShame
@onready var bundle_chaos: Button = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelFilters/VBoxContainer/VBoxContainer/HBoxContainer/boxBundles/bunChaos
@onready var bundle_duty: Button = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelFilters/VBoxContainer/VBoxContainer/HBoxContainer/boxBundles/bunDuty

@onready var box_bundles: GridContainer = $Panel/pnlBlackBack/MarginContainer/pnlBorder/VBoxContainer/panelFilters/VBoxContainer/VBoxContainer/HBoxContainer/boxBundles
...
var selected_bundles: Array[Enums.ItemBundles] = [] #[Enums.ItemBundles.GENERAL, Enums.ItemBundles.REVENGE, Enums.ItemBundles.HONOR, Enums.ItemBundles.GREED, Enums.ItemBundles.SHAME, Enums.ItemBundles.DUTY, Enums.ItemBundles.CHAOS]

var item_scene = preload("res://Scenes/item.tscn")
var set_scene = preload("res://Scenes/Elements/set_bonus_display.tscn")

var all_item_nodes: Array = []  # Store references to instantiated item nodes
var all_set_nodes: Array = []  # Store references to instantiated item nodes
...
func populate_all_items(_in_game: bool = false):
	if is_populating:
		return
	...
	var all_items = ItemsManager.get_all_items(_in_game)

	all_items = all_items.filter(func(item: Item) -> bool:
		return item.rarity not in [Enums.Rarity.GOLDEN, Enums.Rarity.DIAMOND]
	)
...
func populate_all_sets():
	...
	var all_sets = SetBonusManager.all_set_bonuses

	# Sort by  Name
	all_sets.sort_custom(func(a: SetBonus, b: SetBonus) -> bool:

		# sory by name
		return a.setbonus_name < b.setbonus_name
	)
		
	var batch_size = 15
	
	for i in range(0, all_sets.size(), batch_size):
		var batch_end = mini(i + batch_size, all_sets.size())
		
		for j in range(i, batch_end):
			var item: Item = all_sets[j].setbonus_item
			var choice_button: SetBonusDisplay = set_scene.instantiate()
			choice_button.custom_minimum_size = Vector2(100, 100)
			set_grid.add_child(choice_button)
			choice_button.set_is_from_compendium(true)
			choice_button.set_bonus(item)
			choice_button.set_recipe(all_sets[j])
	...
	for item_data in all_item_nodes:
		...
		if is_visible and current_rarity_filter != 0:
			if current_rarity_filter == 1 and item.rarity != Enums.Rarity.COMMON:
				...
				is_visible = false
			elif current_rarity_filter == 5 and item.rarity not in [Enums.Rarity.CRAFTED]:  # Add if needed: Enums.Rarity.GOLDEN, Enums.Rarity.DIAMOND
				is_visible = false
			elif current_rarity_filter == 6 and item.rarity in [Enums.Rarity.CRAFTED]: 
				is_visible = false
...
func show_panel(_in_game: bool = false):
	visible = true
	if !has_loaded:
		await populate_all_items(_in_game)
		await populate_all_sets()
		_setup_category_chips()
		_setup_keyword_chips()
		apply_filters()
		connect_bundle_buttons()
		box_bundles.visible = !_in_game
		has_loaded = true
...
func connect_bundle_buttons():
	...
	bundle_shame.pressed.connect(_on_bundle_toggle.bind(Enums.ItemBundles.SHAME))
	bundle_duty.pressed.connect(_on_bundle_toggle.bind(Enums.ItemBundles.DUTY))
	bundle_chaos.pressed.connect(_on_bundle_toggle.bind(Enums.ItemBundles.CHAOS))
